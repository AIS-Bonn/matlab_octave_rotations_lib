...
function PlotCsys(R, label, P, L, varargin)
	...
	plot3(GXAxis(:,1),GXAxis(:,2),GXAxis(:,3),'b-',varargin{:}); % x => Blue
	washeld = ishold;
	hold on;
	plot3(GYAxis(:,1),GYAxis(:,2),GYAxis(:,3),'-','Color',[0.15 0.70 0.15],varargin{:}); % y => Green
	plot3(GZAxis(:,1),GZAxis(:,2),GZAxis(:,3),'r-',varargin{:}); % z => Red
	if any(label ~= 0) || isempty(label)
		...
		text(GTLbl(1,1),GTLbl(1,2),GTLbl(1,3),xlbl,'Color','b','HorizontalAlignment','center'); % x => Blue
		text(GTLbl(2,1),GTLbl(2,2),GTLbl(2,3),ylbl,'Color',[0.15 0.70 0.15],'HorizontalAlignment','center'); % y => Green
		text(GTLbl(3,1),GTLbl(3,2),GTLbl(3,3),zlbl,'Color','r','HorizontalAlignment','center'); % z => Red
	end
	if ~washeld
		hold off;
	end
