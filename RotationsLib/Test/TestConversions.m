% TestConversions.m - Philipp Allgeuer - 22/10/14
% Tests:   *From*
% Assumes: Rand*, *Equal
%
...
function [Pass] = TestConversions(N, Tol, Inter)
	...
	P = P & EndBoolean(B);
	P = P & EndTest(1e-9, 'Euler 2-conversions', ErrA, 'Fused 2-conversions', ErrB, 'Quat 2-conversions', ErrC, 'Rotmat 2-conversions', ErrD, 'Tilt 2-conversions', ErrE);
...
% 		[~, ErrD(k,1)] = EulerEqual(EulerFromFused(FusedFromRotmat(Rr)), ERr); % Note: The EulerFromFused function is sensitive to noise very near gimbal lock and infrequently gives errors in excess of 1e-10.
	...
	fprintf('Using special tolerance %g for error testing.\n\n', 1e-9);
	P = P & EndBoolean(B);
	P = P & EndTest(1e-9, 'Euler-Quat 3-conversions', ErrA, 'Fused-Rotmat 3-conversions', ErrB, 'Quat-Tilt 3-conversions', ErrC, 'Rotmat-Euler 3-conversions', ErrD, 'Tilt-Fused 3-conversions', ErrE);
